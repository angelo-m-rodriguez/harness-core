...
public class ErrorTrackingFunctor implements LateBindingValue {
  ...
  public Object bind() {
    ...
    try {
      ErrorTrackingDefaultAgentConfig config = SafeHttpCall.execute(errorTrackingClient.getDefaultAgentConfig(accountId, orgId, projectId));
      jsonObject.put("defaultAgentToken", config.getDefaultAgentToken());
      jsonObject.put("collectorUrl", config.getCollectorUrl());
    } catch (Exception ex) {
      throw new EngineFunctorException(String.format("Unable to retrieve CET agent configuration: account=%s, org=%s, project=%s", accountId, orgId, projectId), ex);
    }
